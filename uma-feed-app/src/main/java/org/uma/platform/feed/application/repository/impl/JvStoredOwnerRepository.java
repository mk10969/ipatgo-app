...
public class JvStoredOwnerRepository implements JvLinkStoredRepository<Owner> {
    ...
    public JvStoredOwnerRepository(JvLinkModelMapper jvLinkModelMapper,
    ...
    @Override
    public List<Owner> findAll(LocalDateTime dateTime, Option option) {

        try (Stream<JvStringContent> lines = JvLink.lines(storedOpenCondition, dateTime, option)) {
            return lines
                    .map(jvContent -> jvLinkModelMapper
                            .deserialize(jvContent.getLine(), Owner.class))
                    .collect(ImmutableList.toImmutableList());
        }
    }

    @Override
    public Flux<Owner> readFlux(LocalDateTime dateTime, Option option) {
        return null;
    }
