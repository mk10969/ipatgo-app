...
//    }
    ...
    public static <T extends JvContent> Flux<T> publisher(final Function<JvLinkWrapper, Flux<T>> function) {
        Objects.requireNonNull(function);
        synchronized (jvlink) {
            return function.apply(jvlink)
    ...
    public static Stream<JvStringContent> lines(final StoredOpenCondition condition,
        ...
        return builder(jvLink ->
                jvLink.init()
                        ...
                        .read(condition)
                        .stream()
                        .filter(content -> condition.getRecordType().getCode()
                                .startsWith(content.getLine()))
        );
    }
    ...
    public static Flux<JvStringContent> fetch(final StoredOpenCondition condition,
                                              final LocalDateTime fromTime,
                                              final Option option) {
    ...
    public static Flux<JvStringContent> fetch(final RealTimeOpenCondition condition,
                                              final RealTimeKey rtKey) {
        return publisher(jvLink ->
                jvLink.init()
                        .rtOpen(condition, rtKey)
     ...
     * @param line
     */
    public static void writer(final Path filePath, final Charset cs, final String line) throws IOException {
    ...
    public Mono<Void> checkFilePath(final Path filePath, final Charset cs, final String line) {
